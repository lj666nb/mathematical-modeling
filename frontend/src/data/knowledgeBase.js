...
print(f"最优值: {result.fun:.4f}, 迭代: {result.nit}")`,
        steps: [
          '选择初始内点 x>0 和障碍参数 mu0',
          '用牛顿法求解当前mu对应的KKT条件',
          '减小mu = sigma*mu（sigma=0.1）',
          '以前一步解为初始点重复',
          'mu足够小时停止'
        ],
        useCases: [
...
print(f"函数值: {best_f:.6f} (理论最小值=0)")`,
      ...
      {
        name: '模拟退火',
        brief: '模拟物理退火过程，以一定概率接受劣解跳出局部最优',
...
print(f"最短距离: {best_d:.1f}")`,
      ...
      {
        name: '粒子群优化',
        brief: '模拟鸟群觅食的群体智能算法，粒子共享信息协同搜索',
...
print(f"最优值: {pbest_f.min():.6f} (理论最小值=0)")`,
  ...
  "prediction": {
    ...
    algorithms: [
      {
        name: 'ARIMA/SARIMA',
        brief: '最经典的时间序列预测模型，通过差分和自回归移动平均建模',
...
print(f"未来8期预测: {forecast.round(1)}")`,
      ...
      {
        name: 'Prophet',
        brief: 'Facebook开源的时间序列预测工具，自动处理节假日和变点',
...
print(forecast[["ds", "yhat", "yhat_lower", "yhat_upper"]].tail())`,
        steps: [
          '准备两列数据：ds(日期)和y(目标值)',
          '创建Prophet模型，指定季节性和节假日',
          'model.fit()训练模型',
          'model.make_future_dataframe()创建未来日期',
          'model.predict()生成预测和置信区间',
          'model.plot_components()可视化趋势/季节分解'
        ],
        useCases: [
          '业务KPI预测',
          '网站流量预测',
      ...
      {
        name: 'LSTM/GRU',
        brief: '深度学习时序预测模型，擅长捕捉长期依赖和非线性模式',
...
print(f"测试集MSE: {mse.item():.4f}")`,
        steps: [
          '数据预处理：归一化/标准化到[0,1]或N(0,1)',
          '创建滑动窗口：用过去N个时间步预测未来M步',
          '设计LSTM网络：输入层→LSTM层(含Dropout)→全连接输出层',
          '训练：Adam优化器+MSE损失，早停防止过拟合',
          '评估：在测试集上计算RMSE/MAE/MAPE'
        ],
        useCases: [
          '股票价格预测',
          '能源负荷预测',
...
print(f"测试RMSE: {np.sqrt(mean_squared_error(y_target[train_n:], pred)):.3f}")`,
        steps: [
          '特征工程：构造滞后特征(lag1/lag7/lag30)、滚动统计(mean/std)、时间特征(月/星期/季度)',
          '划分训练集和测试集（时序数据不能随机划分！）',
          '训练XGBoost模型并调参（max_depth/learning_rate/n_estimators）',
          '特征重要性分析，筛选有效特征',
          '在测试集上评估，必要时做残差分析'
        ],
        useCases: [
  ...
  "evaluation": {
    ...
    algorithms: [
      {
        name: 'TOPSIS',
        brief: '逼近理想解排序法，根据方案与正负理想解的距离进行排序',
...
for i, r in enumerate(rank):
    print(f"排名{r}: 供应商{i+1} (贴近度={S[i]:.4f})")`,
      ...
      {
        name: '熵权法',
        brief: '基于数据信息熵的客观赋权方法，指标差异越大权重越高',
...
for j, (e, w) in enumerate(zip(E, W)):
    print(f"指标{j+1}: 熵={e:.4f}, 权重={w:.4f}")`,
        steps: [
          ...
          '计算每个指标的信息熵E_j',
          '计算信息效用值d_j=1-E_j',
          '归一化得到权重w_j=d_j/sum(d_j)'
        ],
        useCases: [
          '多指标综合评价中的权重确定',
          '与TOPSIS/VIKOR等方法配合使用',
          '特征筛选（剔除低权重指标）'
        ],
...
print(f"特征系数: {model.coef_[0].round(4)}")`,
      ...
      {
        name: 'K-means聚类',
        brief: '最经典的聚类算法，将数据划分为K个簇，最小化簇内平方和',
...
print(f"质心: {kmeans.cluster_centers_.round(3)}")`,
      ...
      {
        name: 'DBSCAN',
        brief: '基于密度的聚类算法，可发现任意形状簇并自动识别噪声',
...
print(f"最终种群: {ys[-1]:.0f} (容量K=1000)")`,
        steps: [
          '确定微分方程 dy/dt = f(t,y) 和初值 y(t0)=y0',
          '选择步长h（步长越小精度越高但计算量越大）',
          '从t0开始，y_{n+1}=y_n+h*f(t_n,y_n)迭代',
          '直到到达终止时间t_end'
        ],
        useCases: [
          '教学演示',
...
print(f"感染峰值: 第{peak_day:.0f}天, 最大感染人数: {ys[:,1].max():.0f}")`,
        steps: [
          '定义ODE系统 y\'=f(t,y) 和初值',
          '选择步长h（常用0.01-0.1）',
          '每步计算k1(起点斜率)、k2/k3(中点斜率)、k4(终点斜率)',
          '加权平均更新y_{n+1}',
          '重复直到终止时间'
        ],
        useCases: [
      ...
      {
        name: '有限差分法',
        brief: '求解偏微分方程(PDE)的核心数值方法，将导数离散化为差分',
...
print(f"最终温度分布: 中心={temp[nx//2]:.3f}, 边缘={temp[0]:.3f}")`,
        steps: [
          '确定求解域和边界条件（Dirichlet/Neumann）',
          '网格剖分：空间步长dx和时间步长dt',
          '选择差分格式（显式/隐式/Crank-Nicolson）',
          '稳定性条件检查（显式FTCS: dt <= dx^2/(2*alpha)）',
          '逐时间步求解线性方程组'
        ],
        useCases: [
          '热传导模拟',
  ...
  "statistics": {
    ...
    algorithms: [
      {
        name: 'OLS回归',
        brief: '最小二乘线性回归，通过最小化残差平方和估计回归系数',
...
print(model.summary().tables[1])  # 系数表`,
      ...
      {
        name: '主成分分析(PCA)',
        brief: '最常用的降维方法，将高维数据投影到方差最大的方向',
...
print(f"前3个成分累计: {pca.explained_variance_ratio_[:3].sum():.2%}")`,
      ...
      {
        name: '贝叶斯推断',
        brief: '基于贝叶斯定理的统计推断方法，融合先验知识和观测数据',
...
print(f"B转化率高于A的概率: {prob_b_better:.1%}")`,
        steps: [
          '确定似然函数（数据生成过程）',
          '选择先验分布（无信息先验/共轭先验/领域知识）',
          '计算后验分布（解析解/MCMC采样/变分推断）',
          '后验分析：均值/中位数/可信区间',
          '后验预测检查验证模型'
        ],
        useCases: [
...
print("最短路径:", " -> ".join(path), "距离=", dist)`,
        steps: [
          '初始化：源点距离=0，其他=无穷，所有节点未访问',
          '选择未访问节点中距离最小的u',
          '标记u为已访问',
          '对u的每个邻居v：若d[u]+w(u,v) < d[v]，更新d[v]',
          '重复2-4直到目标节点被访问'
        ],
        useCases: [
          '地图导航（GPS路径规划）',
      ...
      {
        name: 'PageRank',
        brief: 'Google创始算法，通过随机游走模型评估节点重要性',
...
for node, score in sorted(pr.items(), key=lambda x: -x[1]):
    print(f"节点{node}: PageRank={score:.4f}")`,
        steps: [
          '构建有向图（节点=网页/用户，边=链接/关注关系）',
          '初始化所有节点PR值为1/N',
          '迭代更新：新PR = (1-d)/N + d*sum(入链PR/出度数)',
          '重复直到收敛（PR变化<阈值）'
        ],
        useCases: [
          '搜索引擎网页排序',
...
print(f"95% VaR: {VaR_95:.2%} (有95%把握年损失不超过{-VaR_95:.2%})")`,
      ...
      {
        name: 'MCMC',
        brief: '马尔可夫链蒙特卡洛，从复杂后验分布中采样的核心工具',
...
print(f"95%可信区间: [{np.percentile(posterior, 2.5):.3f}, {np.percentile(posterior, 97.5):.3f}]")`,
        steps: [
          '定义目标分布（后验=似然×先验）',
          '选择提议分布（常用正态随机游走）',
          '采样：提议新状态→计算接受概率→接受/拒绝',
          '预烧期(burn-in)：丢弃前若干迭代（链未收敛）',
          '收敛诊断：多链R_hat、迹图、自相关图',
          '后验分析：基于后验样本计算均值/分位数等'
        ],
        useCases: [
          '贝叶斯参数估计',
          '复杂后验分布采样',
...
/**
 * 获取某个分类下的某个算法笔记（按名称匹配）
 */
export function getAlgorithmNote(categoryId, algorithmIndex, algorithmName) {
  const cat = ALGORITHM_NOTES[categoryId]
  if (!cat || !algorithmName) return null
  // 按名称精确匹配
  return cat.algorithms.find(a => a.name === algorithmName) || null
}
